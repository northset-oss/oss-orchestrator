...
async function temporaryLake(t) {
  ...
  db.exec(`
    ...
    CREATE UNIQUE INDEX repositories_node_id_idx
      ON repositories(repository_node_id) WHERE repository_node_id IS NOT NULL;
    CREATE TABLE repository_aliases(
      alias_key TEXT PRIMARY KEY,repo_key TEXT NOT NULL,repository_node_id TEXT,
      observed_at TEXT NOT NULL,FOREIGN KEY(repo_key) REFERENCES repositories(repo_key)
    );
    CREATE TABLE issues(
...
test('renamed repository and issue node IDs reuse existing lake identities without duplicate work', async (t) => {
  const lake = await temporaryLake(t);
  const original = issue('Original/Repo', 7);
  const first = await discoverCandidates({
    lakePath: lake, target: 1, now: NOW,
    search: async ({stratum}) => ({
      body: {data: {search: {nodes: stratum.name === 'good_first_issue:global' ? [original] : []}}},
    }),
  });
  assert.equal(first.inserted, 1);

  const renamed = issue('Renamed/Repo', 7);
  renamed.repository.id = original.repository.id;
  renamed.id = original.id;
  const second = await discoverCandidates({
    lakePath: lake, target: 1, now: NOW,
    search: async ({stratum}) => ({
      body: {data: {search: {nodes: stratum.name === 'good_first_issue:global' ? [renamed] : []}}},
    }),
  });
  ...
  const db = new DatabaseSync(lake, {readOnly: true});
  assert.deepEqual({...db.prepare(`SELECT repo_key,repo_display,repository_node_id
    FROM repositories`).get()}, {
    repo_key: 'original/repo',
    repo_display: 'Renamed/Repo',
    repository_node_id: original.repository.id,
  });
  assert.deepEqual({...db.prepare(`SELECT candidate_key,candidate_display,repo_key,issue_node_id
    FROM issues`).get()}, {
    candidate_key: 'original/repo#7',
    candidate_display: 'Renamed/Repo#7',
    repo_key: 'original/repo',
    issue_node_id: original.id,
  });
  assert.deepEqual({...db.prepare(`SELECT alias_key,repo_key,repository_node_id
    FROM repository_aliases WHERE alias_key='renamed/repo'`).get()}, {
    alias_key: 'renamed/repo',
    repo_key: 'original/repo',
    repository_node_id: original.repository.id,
  });
  db.close();
...
test('a reused repository name preserves old node identities and retires stale candidates', async (t) => {
  const lake = await temporaryLake(t);
  const original = issue('Original/Repo', 7);
  ...
  const replacement = issue('Original/Repo', 7);
  replacement.repository.id = 'R_reused_name';
  replacement.id = 'I_reused_number';
  const result = await discoverCandidates({
    lakePath: lake, target: 1, now: NOW,
    search: async ({stratum}) => ({
      body: {data: {search: {nodes: stratum.name === 'good_first_issue:global' ? [replacement] : []}}},
    }),
  });
  ...
  const db = new DatabaseSync(lake, {readOnly: true});
  assert.deepEqual(db.prepare(`SELECT repo_key,repository_node_id FROM repositories
    ORDER BY repo_key`).all().map((row) => ({...row})), [{
    repo_key: 'node:R_reused_name',
    repository_node_id: 'R_reused_name',
  }, {
    repo_key: 'original/repo',
    repository_node_id: original.repository.id,
  }]);
  assert.deepEqual(db.prepare(`SELECT candidate_key,candidate_display,repo_key,state,issue_node_id
    FROM issues ORDER BY candidate_key`).all().map((row) => ({...row})), [{
    candidate_key: 'node:I_reused_number',
    candidate_display: 'Original/Repo#7',
    repo_key: 'node:R_reused_name',
    state: 'OPEN',
    issue_node_id: 'I_reused_number',
  }, {
    candidate_key: 'original/repo#7',
    candidate_display: 'Original/Repo#7',
    repo_key: 'original/repo',
    state: 'RENAMED_UNKNOWN',
    issue_node_id: original.id,
  }]);
  assert.deepEqual({...db.prepare(`SELECT alias_key,repo_key,repository_node_id
    FROM repository_aliases WHERE alias_key='original/repo'`).get()}, {
    alias_key: 'original/repo',
    repo_key: 'node:R_reused_name',
    repository_node_id: 'R_reused_name',
  });
  db.close();
...
test('name reuse does not retire issues already refreshed to a renamed live repository', async (t) => {
  const lake = await temporaryLake(t);
  const original = issue('Original/Repo', 7);
  await discoverCandidates({
    lakePath: lake, target: 1, now: NOW,
  ...
  const replacement = issue('Original/Repo', 7);
  replacement.repository.id = 'R_reused_after_rename';
  replacement.id = 'I_reused_after_rename';
  await discoverCandidates({
    lakePath: lake, target: 1, now: NOW,
    search: async ({stratum}) => ({
      body: {data: {search: {nodes: stratum.name === 'good_first_issue:global' ? [replacement] : []}}},
  ...
  const db = new DatabaseSync(lake, {readOnly: true});
  assert.deepEqual(db.prepare(`SELECT candidate_key,candidate_display,state,issue_node_id
    FROM issues ORDER BY candidate_key`).all().map((row) => ({...row})), [{
    candidate_key: 'node:I_reused_after_rename',
    candidate_display: 'Original/Repo#7',
    state: 'OPEN',
    issue_node_id: 'I_reused_after_rename',
  }, {
    candidate_key: 'original/repo#7',
    candidate_display: 'Renamed/Repo#7',
    state: 'OPEN',
...
test('name reuse retires only stale siblings after a partial repository rename refresh', async (t) => {
  const lake = await temporaryLake(t);
  const first = issue('Original/Repo', 7);
  const staleSibling = issue('Original/Repo', 8);
  await discoverCandidates({
    lakePath: lake, target: 2, now: NOW,
    search: async ({stratum}) => ({
      body: {data: {search: {
        nodes: stratum.name === 'good_first_issue:global' ? [first, staleSibling] : [],
      }}},
    }),
  });

  const refreshed = issue('Renamed/Repo', 7);
  refreshed.repository.id = first.repository.id;
  refreshed.id = first.id;
  await discoverCandidates({
    lakePath: lake, target: 1, now: NOW,
    search: async ({stratum}) => ({
      body: {data: {search: {nodes: stratum.name === 'good_first_issue:global' ? [refreshed] : []}}},
    }),
  });

  const replacement = issue('Original/Repo', 9);
  replacement.repository.id = 'R_partial_reuse';
  replacement.id = 'I_partial_reuse';
  await discoverCandidates({
    lakePath: lake, target: 1, now: NOW,
    search: async ({stratum}) => ({
      body: {data: {search: {nodes: stratum.name === 'good_first_issue:global' ? [replacement] : []}}},
  ...
  const db = new DatabaseSync(lake, {readOnly: true});
  assert.deepEqual(db.prepare(`SELECT candidate_key,candidate_display,state FROM issues
    ORDER BY candidate_key`).all().map((row) => ({...row})), [{
    candidate_key: 'original/repo#7',
    candidate_display: 'Renamed/Repo#7',
    state: 'OPEN',
  }, {
    candidate_key: 'original/repo#8',
    candidate_display: 'Original/Repo#8',
    state: 'RENAMED_UNKNOWN',
  }, {
    candidate_key: 'original/repo#9',
    candidate_display: 'Original/Repo#9',
    state: 'OPEN',
  }]);
  db.close();
});

test('legacy rows without node IDs fail closed when a live identity occupies their names', async (t) => {
  const lake = await temporaryLake(t);
  const db = new DatabaseSync(lake);
  db.prepare(`INSERT INTO repositories(
    repo_key,repo_display,test_profile,raw_json,provenance_json,updated_at
  ) VALUES(?,?,?,?,?,?)`).run(
    'legacy/repo', 'Legacy/Repo', 'node', '{}', '{}', NOW.toISOString(),
  );
  db.prepare(`INSERT INTO issues(
    candidate_key,candidate_display,repo_key,issue_number,state,labels_json,assignees_json,
    profile,mechanical_reasons_json,raw_json,updated_at
  ) VALUES(?,?,?,?,?,?,?,?,?,?,?)`).run(
    'legacy/repo#4', 'Legacy/Repo#4', 'legacy/repo', 4, 'OPEN', '[]', '[]',
    'node', '[]', '{}', NOW.toISOString(),
  );
  db.close();

  const live = issue('Legacy/Repo', 4);
  const result = await discoverCandidates({
    lakePath: lake, target: 1, now: NOW,
    search: async ({stratum}) => ({
      body: {data: {search: {nodes: stratum.name === 'good_first_issue:global' ? [live] : []}}},
    }),
  });
  assert.equal(result.inserted, 1);

  const stored = new DatabaseSync(lake, {readOnly: true});
  assert.deepEqual(stored.prepare(`SELECT repo_key,repository_node_id FROM repositories
    ORDER BY repo_key`).all().map((row) => ({...row})), [{
    repo_key: 'legacy/repo',
    repository_node_id: null,
  }, {
    repo_key: `node:${live.repository.id}`,
    repository_node_id: live.repository.id,
  }]);
  assert.deepEqual(stored.prepare(`SELECT candidate_key,state,issue_node_id FROM issues
    ORDER BY candidate_key`).all().map((row) => ({...row})), [{
    candidate_key: 'legacy/repo#4',
    state: 'RENAMED_UNKNOWN',
    issue_node_id: null,
  }, {
    candidate_key: `node:${live.id}`,
    state: 'OPEN',
    issue_node_id: live.id,
  }]);
  stored.close();
});

test('known issue node IDs prevent renamed factory work from being rediscovered', async (t) => {
  const lake = await temporaryLake(t);
  const renamed = issue('Renamed/Repo', 7);
  const result = await discoverCandidates({
    lakePath: lake,
  ...
  assert.equal(result.skipped_reason_counts['issue node is already known to factory'], 1);
});
