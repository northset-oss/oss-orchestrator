...
export function verifyReviewSet(manifest, records, roster, {minimumReviewers = 1} = {}) {
  ...
  for (const record of records) {
    ...
    if (record.disposition !== 'SHIP') throw new Error('every shipping review disposition must be SHIP');
  }
...
export function createBatchApproval(manifests, {
  privateKey, approvedDigest, approvedAt = new Date().toISOString(),
} = {}) {
